...
# CONFIGURAÇÕES
# ======================================================
# Sua chave Gemini (Cuidado ao compartilhar)
GEMINI_KEY = "AIzaSyCu7cyG4zuxx8hzG_4nbdsgIaKsfCPxx7k" 

# Sua chave ElevenLabs
...
# IMPORTANTE: Coloque o Token do @BotFather aqui embaixo
TG_BOT_TOKEN = "SEU_TOKEN_DO_BOTFATHER_AQUI"

# Modelo Gemini (Usando o 2.0 Flash Experimental)
MODEL_NAME = "gemini-2.0-flash-exp"

# ======================================================
# BACKEND (FLASK + NOVAS APIs)
# ======================================================
app = Flask(__name__)
CORS(app)
...
def start_telegram():
    ...
    try:
        if TG_BOT_TOKEN != "SEU_TOKEN_DO_BOTFATHER_AQUI":
            ...
            print(">>> AVISO: Configure o Token do Telegram no código!")
...
def get_ai_response(text):
    try:
        # Chamada usando o NOVO SDK do Google
        ...
        # Se der erro de cota (429) no modelo 2.0, avisa o usuário
        if "429" in str(e):
            return "Erro: Cota do Gemini 2.0 excedida. Tente novamente em alguns segundos."
        return f"Erro no processamento neural (Gemini): {e}"

def get_voice(text):
    if not ELEVEN_KEY: return None
    
    # Limita tamanho do texto para economizar créditos ElevenLabs
...
# ======================================================
# FRONTEND (VISUAL KING AI - PRETO E DOURADO)
# ======================================================
HTML_PAGE = """
<!DOCTYPE html>
<html lang="pt-BR">
...
<body class="bg-gradient-to-br from-slate-900 via-black to-slate-800 h-screen w-screen text-white relative">
    ...
    <div id="loginScreen" class="absolute inset-0 z-40 flex items-center justify-center p-4">
        <div class="bg-black/60 backdrop-blur-md p-8 rounded-3xl border border-yellow-500/30 shadow-2xl max-w-md w-full text-center crown-glow">
            ...
            <p class="text-gray-400 mb-2">Gemini 2.0 Flash</p>
